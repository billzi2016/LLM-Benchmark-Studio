...
export const useStudioStore = defineStore('studio', {
  ...
  actions: {
    async loadInitialData() {
      this.loading = true
      try {
        const [systemStatus, providers, models, datasets, languages] = await Promise.all([
          fetchSystemStatus(),
          fetchProviders(),
          fetchModels(),
          fetchDatasets(),
        ...
        if (!this.selectedModelNames.length) {
          this.selectedModelNames = models.filter((model) => model.activate).slice(0, 2).map((model) => model.name)
        }
        if (!this.selectedDatasetNames.length) {
          this.selectedDatasetNames = datasets.slice(0, 3).map((dataset) => dataset.dataset_name)
        }
      } finally {
        this.loading = false
      }
